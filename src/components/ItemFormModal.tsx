  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (photoFile) {
        ...
        // TODO(T6): drop this re-fetch once OrganizingItem carries cutoutURL
        // directly (T6 adds it to types.ts/data/presentation.ts).
        const fresh = await getItem(item.id);
        cutoutURL = fresh.cutoutURL || undefined;
      }
      const payload = {
        name: name.trim(),
